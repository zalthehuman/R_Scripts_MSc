setwd('/Users/zal/Desktop/grad school shit/Thesis Data/Laguna Lake/data/R_Scripts_DA/')# Load necessary library
library(dplyr)

# Assuming data has been loaded and structured as previously described

# Set a base weight
base_weight <- 0.15

# Calculate Dynamic Weight for Algal Count
# This weight decreases the total score as Algal Count increases
data$DynamicWeight_AlgalCount <- base_weight * data$AlgalCount

# Calculate the sum of all other normalized weighted scores
data$Total_Score <- rowSums(data[, c("DO", "Ph", "Temp", "TSS", "BOD", "NH3", "FC", "Chloride", "NO3")])

# Subtract dynamically weighted Algal Count
# Higher Algal Count will now reduce the WQI more significantly
data$Total_Score <- data$Total_Score - data$DynamicWeight_AlgalCount

# Normalize WQI to have a maximum score of 1
# Ensure no negative scores by setting a floor at 0
data$WQI <- pmax(data$Total_Score / max(data$Total_Score), 0)

# Output the data to check
print(data[, c("AlgalCount", "DynamicWeight_AlgalCount", "Total_Score", "WQI")])


# Load the water quality data into R as a data frame
water_data <- read.csv("imputedStn1.csv")
...
ggplot(water_data, aes(x = Biological)) + 
  geom_histogram(bins = 8, fill = "brown", color = "white") + 
